...
public class ControleGeral {
	...
	private List<String> lerArquivoJogadores(final String caminho) throws Exception {		
		...
		//arquivo linux
			...
			//arquivo windows
			//File file = new File("E:\\jogadores.txt");
			...
			while ((st = br.readLine()) != null) {
				...
				for (int i = 0; i < st.length(); i++) {      
					...
					//se for espaco em branco ou numero
					//if(Character.isWhitespace(st.charAt(i)) || Character.isDigit(st.charAt(i))) {
					//	continue;
					//} 					
					//else //verifica caracter separador
					if(st.charAt(i) == ';') {
					...
					else //elimina caracteres especiais 
	...
	private List<Selecao> lerArquivoSelecoes(final String caminho) throws Exception {
		...
		//hashmap para inserir no bd
		//Map<Integer, String> selecao = new HashMap<>();
